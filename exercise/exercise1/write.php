...
if(!empty($_POST['receiver']) && !empty($_POST['subject']) && !empty($_POST['text']))
{
	...
	$user_id = $pdo->query("SELECT id FROM exercise1_user WHERE username=".$receiver)->fetch();
	
	if(!empty($user_id['id']))
	{
		$id = intval($user_id['id']);
		
		$pdo->exec("INSERT INTO exercise1_message VALUES(NULL, ".intval(getUserId()).", ".$id.", ".$subject.", ".$text.")");

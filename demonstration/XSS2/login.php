<?php
	...
	if(!empty($_POST['username']) && !empty($_POST['password']))
	{
		...
		$user = $pdo->query("SELECT id FROM demonstrationXSS2_user WHERE username=".$username." AND password=".$password)->fetch();
		...
		if(!empty($user['id']))
		{
			...
			if($_POST['stay'] == 'on')
			{
				...
				$key = $pdo->quote($key);
				$pdo->exec("UPDATE demonstrationXSS2_user SET token=".$key." WHERE id=".intval($user['id']));
			}
